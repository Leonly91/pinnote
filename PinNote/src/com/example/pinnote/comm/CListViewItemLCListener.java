...
public class CListViewItemLCListener extends ListViewItemLongClickBaseListener{
	
	private ListView listView;
	private Context context;
	private NoteType noteType;

	public CListViewItemLCListener(ListView listView, NoteType noteType,
			Context context) {
	...
	public void regDialogEvent(Dialog dialog, Note note) {
		...
		OnClickListener itemBtnOnClickListener = new CItemBtnClickListener(dialog, note);
		...
		fileBtn.setOnClickListener(itemBtnOnClickListener);
		doBtn.setVisibility(View.GONE);
		doneBtn.setVisibility(View.GONE);
	}
	
	class CItemBtnClickListener implements OnClickListener
	{
		private Dialog dialog;
		private Note note;
		
		public CItemBtnClickListener(Dialog dialog, Note note){
			this.dialog = dialog;
			this.note = note;
		}
		...
		public void onClick(View v) {
			...
			case R.id.pinBtn:
				//pinNote(noteId)
				break;
			case R.id.fileNoteBtn:
				DBUtil.updateNoteType(context, note, NoteType.FILE);
				
				//switch to done listview tab
				note_data = DBUtil.getDoneNoteArray(context);
	        	if (null != note_data)
				{
	        		...
	        		Toast.makeText(context, "ÒÑ¹éµµ", Toast.LENGTH_SHORT).show();
				}
